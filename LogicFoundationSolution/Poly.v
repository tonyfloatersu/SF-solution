...
Proof.
  ...
  - simpl. rewrite -> IH0. rewrite -> app_assoc. reflexivity. Qed.
...
Definition filter_even_gt7 (l : list nat) : list nat := filter (fun n => evenb n && blt_nat 7 n) l.
...
Definition partition {X : Type}
                     (test : X -> bool)
                     (l : list X) : list X * list X := (filter test l,
...
Lemma map_cat : forall (X Y : Type) (f : X -> Y) (l1 l2 : list X),
    map f (l1 ++ l2) = map f l1 ++ map f l2.
Proof.
  intros X Y f l1 l2.
  induction l1 as [ | hl10 l10 IH0 ].
  - simpl. reflexivity.
  - simpl. rewrite -> IH0. reflexivity. Qed.
...
Proof.
  ...
  - reflexivity.
  - simpl. rewrite -> map_cat. rewrite -> IH0. reflexivity. Qed.
(** [] *)
