...
(** Fixpoint increaseTo (n m : nat) : nat :=
     match (blt_nat n m) with
     | true => increaseTo (S n) m
     | false => n
     end. *)
...
Inductive binNet : Type :=
| Z : binNet
| T : binNet -> binNet
| M : binNet -> binNet.

Fixpoint incr (b : binNet) : binNet :=
  match b with
  | Z => M Z
  | T b0 => M b0
  | M b1 => T (incr b1)
...
Fixpoint bin_to_nat (b : binNet) : nat :=
  match b with
  | Z => O
  | T b0 => 2 * (bin_to_nat b0)
  | M b1 => 2 * (bin_to_nat b1) + 1
  end.

Example test_bin_incr1 : bin_to_nat (incr Z) = 1.
Proof. reflexivity. Qed.
Example test_bin_incr2 : incr (T Z) = M Z.
Proof. reflexivity. Qed.
Example test_bin_incr3 : (T (T (M Z))) = incr (incr (incr (incr Z))).
Proof. reflexivity. Qed.
Example test_bin_incr4 : bin_to_nat (M (incr (incr Z))) = 5.
Proof. reflexivity. Qed.
Example test_bin_incr5 : incr (M (M (M (M Z)))) = T (T (T (T (M Z)))).
Proof. reflexivity. Qed.
(** [] *)
