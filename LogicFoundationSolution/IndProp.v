...
Proof.
  ...
  - simpl. apply ev_0.
  - simpl. apply ev_SS. apply IH0. Qed.
(** [] *)
...
Proof.
  ...
  apply evSS_ev in H'. apply H'. Qed.
(** [] *)
...
Proof.
  intros n m Hn Hm. induction Hn as [ | n0 Hn0 IH0 ].
  - simpl. apply Hm.
  - simpl. apply ev_SS. apply IH0. Qed.
(** [] *)
...
Proof.
  intros n. split.
  - intros H. induction H as [ | | n0 m0 Hn' Hn Hm' Hm ].
    + apply ev_0.
    + apply (ev_SS 0 ev_0).
    + apply ev_sum.
      * apply Hn.
      * apply Hm.
  - intros H. induction H as [ | n0 H0 IH0 ].
    + apply ev'_0.
    + assert (H : forall k : nat, (S (S k)) = 2 + k).
      { symmetry. apply plus_1_l. }
...
Theorem ev_ev__ev : forall n m,
  ev (n+m) -> ev n -> ev m.
Proof.
  intros n m Hnm Hn. induction Hn as [ | n0 Hn0 IH0 ].
  - simpl in Hnm. apply Hnm.
  - apply IH0. simpl in Hnm. apply evSS_ev. apply Hnm. Qed.
(** [] *)
...
Proof.
  intros n m p Hnm. apply ev_ev__ev. rewrite plus_comm. rewrite plus_swap.
  rewrite <- plus_assoc. rewrite <- double_plus. rewrite plus_assoc. apply ev_sum.
  - apply Hnm.
  - apply ev_double. Qed.
(** [] *)
