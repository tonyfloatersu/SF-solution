...
Proof.
  ...
  - simpl. rewrite -> IH0.
    reflexivity. Qed.
(* GRADE_THEOREM 0.5: mult_0_rewrite  *)
...
Both are used to perform case analysis on an element of an inductively defined type;
...
Proof.
  ...
  induction n as [| n' IH0].
  - reflexivity.
  - simpl. rewrite <- plus_n_Sm. rewrite -> IH0.
    reflexivity. Qed.
(** [] *)
...
Theorem binary_commute : forall b : binNet, bin_to_nat b + 1 = bin_to_nat (incr b).
Proof.
  intros b.
  induction b as [| bT | bM IH0].
  - reflexivity.
  - reflexivity.
  - simpl. rewrite <- IH0. rewrite <- plus_n_O. rewrite <- plus_n_O.
    rewrite -> plus_swap.
    rewrite -> plus_assoc.
    rewrite -> plus_assoc.
    reflexivity. Qed.
...
Fixpoint nat_to_bin (n : nat) : binNet :=
  match n with
  | O => Z
  | S n' => incr (nat_to_bin n')
  end.

Definition normalize (b : binNet) := nat_to_bin (bin_to_nat b).

Theorem conv_nbn : forall n : nat, bin_to_nat (nat_to_bin n) = n.
Proof.
  intros n.
  induction n as [| n' IH0].
  - reflexivity.
...
Theorem conv_bnb : forall b : binNet, nat_to_bin (bin_to_nat b) = normalize b.
Proof.
  intros b.
  reflexivity.  Qed.
(* FILL IN HERE *)
