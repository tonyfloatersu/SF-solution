...
Proof.
  intros p. destruct p as [p0 p1]. simpl. reflexivity. Qed.
(** [] *)
...
Fixpoint oddmembers (l:natlist) : natlist :=
  match l with
  | nil => nil
  | t :: l1 => match oddb t with
              | true => t :: oddmembers l1
              | false => oddmembers l1
              end
  end.
...
Fixpoint alternate (l1 l2 : natlist) : natlist :=
  match l1, l2 with
  | h1 :: l01, h2 :: l02 => h1 :: h2 :: alternate l01 l02
  | l1, nil => l1
  | nil, l2 => l2
  end.
...
Fixpoint subset (s1:bag) (s2:bag) : bool :=
  match s1 with
  | h :: t => match beq_nat (count h s2) 0 with
             | true => false
...
Theorem bag_theorem : forall (n : nat) (l : natlist), S (count n l) = count n (add n l).
Proof.
  intros n l.
  assert (beq_nat n n = true) as H.
  { induction n as [|n' IH0].
    - reflexivity.
    - simpl.
      rewrite -> IH0.
      reflexivity. }
  induction l as [| h0 l0 IH0].
  - simpl. rewrite -> H. reflexivity.
  - simpl. rewrite -> IH0. rewrite -> H. reflexivity. Qed.
...
Theorem app_nil_r : forall l : natlist,
  l ++ [] = l.
Proof.
  intros l. induction l as [|h' l' IH0].
  - reflexivity.
...
Proof.
  intros l1 l2. induction l1 as [| h1' l1' IH01].
  - simpl. rewrite -> app_nil_r. reflexivity.
  - simpl. rewrite -> IH01. rewrite -> app_assoc. reflexivity. Qed.
...
Proof.
  intros l.
  assert (forall n : nat, rev [n] = [n]) as TH.
  { simpl. reflexivity. }
  induction l as [| h l' IH0].
  - reflexivity.
  - simpl. rewrite -> rev_app_distr. rewrite -> IH0.
    rewrite -> TH. reflexivity. Qed.
...
Proof.
  intros l1 l2. induction l1 as [| h1' l1' IH0].
  - simpl. reflexivity.
  - simpl. rewrite -> IH0. destruct h1'.
    { reflexivity. }
    { reflexivity. } Qed.
(** [] *)
...
Proof.
  intros s. induction s as [| h' s' IH0].
  - reflexivity.
...
Theorem bag_count_sum : forall (n : nat) (l1 l2 : bag), count n (sum l1 l2) = count n l1 + count n l2.
Proof. intros n l1 l2.
  induction l1 as [| h1' l1' IH0].
  - simpl. reflexivity.
  - simpl. destruct (beq_nat n h1').
    { rewrite -> plus_Sn_m. rewrite -> IH0. reflexivity. }
    { rewrite -> IH0. reflexivity. } Qed.
...
Proof.
  intros x. destruct x.
  simpl. assert (true = beq_nat n n) as H. { rewrite <- beq_nat_refl. reflexivity. }
  rewrite -> H. reflexivity. Qed.
(** [] *)
...
Proof.
  intros d x v.
  simpl. assert (beq_id x x = true) as H. { rewrite <- beq_id_refl. reflexivity. }
  rewrite -> H. reflexivity. Qed.
(** [] *)
